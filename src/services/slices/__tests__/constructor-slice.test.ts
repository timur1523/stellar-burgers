...
import constructorReducer, { addIngredient, clearConstructor, removeIngredient, replaceIngredient } from "../constructor-slice"
...
describe("constructorSlice", () => {
    ...
    it("Добавление ингредиента в коструктор", () => {
        ...
        expect(state.ingredients[0]).toMatchObject({_id: sause._id, name: sause.name})
    ...
    it("Изменение порядка ингредиентов в конструкторе", () => {
        ...
        const state = constructorReducer(inititalState, replaceIngredient({fromIndex: 0, toIndex: 1}))
